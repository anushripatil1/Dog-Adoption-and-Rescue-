...
class dataBase extends Array {
	...
	public void create() {
		...
		Array dog8 = new Array(109, "Bella Rose", 8, "Indie", 1, "Incompatible with owner lifestyle", 1, "Female", "Brown");
	...
	public void addToDataBase() {
		...
		while(flag2 == 0) {
			...
			if(rescuecenteridofdog<0 && rescuecenteridofdog>3) {
				flag2 = 1;
				break;
			}
	...
	public void displayDataBase() {
		...
		f1.format("%20s %20s %20s %20s %20s %20s %20s\n", "ID", "Name", "Age", "Breed", "Gender", "Colour","Rescue Centre");
		System.out.println(f1);
		System.out.println(
				"____________________________________________________________________________________________________________________________________");

		for (int i = 0; i <= counter; i++) {
			f2.format("%20s %20s %20s %20s %20s %20s %20s\n", dataBase[i].dogId, dataBase[i].name, dataBase[i].age,
					dataBase[i].breed, dataBase[i].gender, dataBase[i].colour,dataBase[i].rescueCentreID);
		}
	...
	public void age(Array[] arr) {
		...
		System.out.println(
				"____________________________________________________________________________________________________________________________________");
		f5.format("%20s %20s %20s %20s %20s %20s %20s \n", "ID", "Name", "Age", "Breed", "Gender", "Colour",
				"Rescue Centre");
		...
		for (int i = 0; i < counter1; i++) {
			if (arr[i] == null) {
				...
				f6.format("%20s %20s %20s %20s %20s %20s\n", arr[i].dogId, arr[i].name, arr[i].age, arr[i].breed, arr[i].gender,
						arr[i].colour, arr[i].rescueCentreID);
	...
	void deletion() {
		...
		int flag1=0;
		while(flag1 == 0) {
			System.out.println("Enter the dog Id of dog you wish to select : ");
			dogIdToBeDeleted = sc.nextInt();

			for(int m=0;m<=counter;m++) {
				if(dogIdToBeDeleted == dataBase[m].dogId) {
					flag1=1;
				}
			}
			if(flag1 == 1) {
...
class Donate {
	...
	public void pay() {
		...
		// list of some donors
		DonateNode newDonor1 = new DonateNode("Anushri Patil", 1234567891, 10000);
		head = newDonor1;
		...
		newDonor4 = new DonateNode("Aditi Wagh", 1234567891, 80000);
		...
		try {
			...
			do{
							...
							do
							{
								...
								case 2:
									...
									do{
										...
										// obj.quickSort(obj.breedDataBase, 0, obj.counter1-1, "age");
										...
										for (int k = 0; k < obj.counter1; k++) {
											f6.format("%20s %20s %20s %20s %20s %20s\n",mainBreedDataBase[k].dogId ,mainBreedDataBase[k].name ,mainBreedDataBase[k].age, mainBreedDataBase[k].breed,mainBreedDataBase[k].gender,mainBreedDataBase[k].colour,mainBreedDataBase[k].rescueCentreID);
				...
				case 2:
					do{
						//ADMIN
						System.out.println("1.Display adopted dogs data\n2. Analysis of number of dogs in each rescue centre\n3. Analysis of number of dogs received from different pincodes");
						choice3 = sc2.nextInt();
						switch(choice3)
						{
						case 1:
